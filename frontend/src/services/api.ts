...
export const getDashboards = async (datasetId: number): Promise<Dashboard[]> => {
    const response = await api.get<Dashboard[]>(`/dashboards/${datasetId}`);
    return response.data;
};

export const saveDashboard = async (datasetId: number, name: string, config: any): Promise<Dashboard> => {
...
export const generateForecast = async (datasetId: number, dateCol: string, valueCol: string, periods: number): Promise<any[]> => {
...
export const sendChatMessage = async (datasetId: number, message: string): Promise<{ response: string; plot_config?: any }> => {
    const response = await api.post<{ response: string; plot_config?: any }>(`/chat/${datasetId}`, { message });
    return response.data;
};
