...
from tkinter.filedialog import askopenfilename # To open the file selection dialog
import pandas as pd
from functools import partial # <--- ADD THIS LINE
import tkinter as tk                              #for GUI
from tkinter import *                             #for GUI                                   +---------------------------+
from tkinter import messagebox                    #for GUI messages                          |         A.V.I.S         |
...
from functools import partial                     #for calling button functions              |        Dashvanth        |
...
# A.V.I.S. Global State Variables
AVIS_DATAFRAME = None # Stores the currently loaded DataFrame
X_COL_NAME = None     # Stores the auto-selected X-column name
Y_COL_NAME = None     # Stores the auto-selected Y-column name
APP_THEME = "appconfig"

# ----------------------------------------------------------------------------------
# NEW A.V.I.S. AUTOMATION FUNCTIONS
# ----------------------------------------------------------------------------------
...
def automated_plot(c, df, x_col, y_col):
    ...
    try:
        ...
        # Functionality 5: Simulate Automated Insight Generation
        messagebox.showinfo(
            title="A.V.I.S. Automated Insight", 
            message=f"Automated EDA Complete! Plot Type: {plot_type}.\n\nInsight: Automatically plotted the first two numeric columns ({x_col} vs {y_col}).\n\nNext Step (Phase 3): Integrate SHAP/LLM for advanced narration.",
            icon="info"
        )
        
        # Note: manage_data.move_figure needs to be accessible, or remove this line if it causes errors
        # manage_data.move_figure(fig, 865, 125) 
        plt.show()

    except Exception as e:
        messagebox.showerror("A.V.I.S. Plotting Error", f"Plotting failed: {e}")

def auto_analyze_file(b1, b2, b3, b4):
    """
    Functionality 1 & 3: File Selection, Loading, and Automated EDA Execution.
    """
    global AVIS_DATAFRAME, X_COL_NAME, Y_COL_NAME
    
    # 1. File Selection (Simulates Multi-Format Ingestion)
    file_path = filedialog.askopenfilename(
        defaultextension=".csv",
        filetypes=[("Data Files (40% MVP)", "*.csv *.xlsx"), ("All files", "*.*")]
    )
    
    if not file_path:
        return # User cancelled
        
    # 2. Automated Data Processing (Calls manage_data.load_and_process_data)
    # The actual loading and processing now happens in the manage_data.py file
    df, message, x_col, y_col = manage_data.load_and_process_data(file_path)
    
    if df is None:
        messagebox.showerror("A.V.I.S. Processing Error", message)
        return
        
    # Update global state and columns
    AVIS_DATAFRAME = df
    X_COL_NAME = x_col
    Y_COL_NAME = y_col
    
    # 3. Automated EDA Summary Output
    global text
    text.pack_forget()
    text = Label(font="poppins 10 bold", fg=theme["White"], bg=theme["Black"], 
                 text=f"--- A.V.I.S. Automated EDA ---\n{message}")
    text.pack(fill=X, pady=35, padx=(0, 200))

    # 4. Update the menu to enable AUTOMATED Plotting (Functionality 4)
    b1.config(command=partial(automated_plot, "line", df, X_COL_NAME, Y_COL_NAME), text=f"Auto-Plot: Line ({Y_COL_NAME.title()})", state=NORMAL)
    b2.config(command=partial(automated_plot, "bar", df, X_COL_NAME, Y_COL_NAME), text=f"Auto-Plot: Bar ({Y_COL_NAME.title()})", state=NORMAL)
    b3.config(command=partial(automated_plot, "scatter", df, X_COL_NAME, Y_COL_NAME), text=f"Auto-Plot: Scatter ({Y_COL_NAME.title()})", state=NORMAL)
    b4.config(command=partial(automated_plot, "hist", df, X_COL_NAME, Y_COL_NAME), text=f"Auto-Plot: Hist ({Y_COL_NAME.title()})", state=NORMAL)
    
    messagebox.showinfo("A.V.I.S. Automation Success", "Automated EDA Complete. Please select an 'Auto-Plot' option to visualize the data.", icon="info")

# ----------------------------------------------------------------------------------
# GUEST FUNCTION (Modified to use Automation)
# ----------------------------------------------------------------------------------
...
def guest(b1,b2,b3,b4,preview_image):
    ...
    # ... (Keep resize_image utility function and image loading logic here) ...
    
    # 1. Automation Button (Triggers the entire workflow)
    load_btn = Button(menu, fg=theme["White"], bg=theme["Neutral"], text="1. Load File (CSV/Excel)", 
                      compound='left', width=170, anchor='w', font="poppins 10", 
                      cursor="hand2", padx=10, 
                      command=partial(auto_analyze_file, b1, b2, b3, b4))
                      
    load_btn.pack(pady=(30, 5), padx=15)
    
    # 2. Automated Plotting Options (Initially Disabled - commands are set dynamically)
    b1=Button(menu,fg=theme["White"],bg=theme["Neutral"],text="Auto-Plot: Line Graph",compound='left',width=170,anchor='w',font="poppins 10",cursor="hand2",padx=10, state=DISABLED)
    b2=Button(menu,fg=theme["White"],bg=theme["Neutral"],text="Auto-Plot: Bar Graph",compound='left',width=170,anchor='w',font="poppins 10",cursor="hand2",padx=10, state=DISABLED)
    b3=Button(menu,fg=theme["White"],bg=theme["Neutral"],text="Auto-Plot: Scatter Plot",compound='left',width=170,anchor='w',font="poppins 10",cursor="hand2",padx=10, state=DISABLED)
    b4=Button(menu,fg=theme["White"],bg=theme["Neutral"],text="Auto-Plot: Histogram",compound='left',width=170,anchor='w',font="poppins 10",cursor="hand2",padx=10, state=DISABLED)
    ...
    # ... (rest of menu button packing, like Go To Home) ...
    
    # NOTE: You must also remove the old manual plotting functions if you haven't already.
#===================================================================================================================plot colors
...
def login(b1,b2,b3,b4,preview_image):
    ...
    global title
    title.pack_forget()
    title=Label(form,font="poppins 10",fg=theme["White"],bg=theme["Primary"],text="INTELLIGENT ANALYTICS DASHBOARD")
...
def create(b2,b1,b3,b4,preview_image):
    ...
    # 1. FIX: Move ALL global declarations to the very top to fix the SyntaxError
    global relation
    global text
    global form
    global title

    # The inner function now correctly accepts the Tkinter variables via partial()
    def show_message(u_name_var, pwd_var, country_var, names): 
        # Retrieve the actual input values from the Tkinter variables
        u_name = u_name_var.get()
        pwd = pwd_var.get()
        country = country_var.get()
        
        # Encrypt password before storing (assuming encrypt() is defined elsewhere)
        pwd = encrypt(pwd) 
        
        if u_name in names:
            messagebox.showinfo(title="Username Not Available", message="That username is already taken. Try another one.",icon="info")
        else:
            # FIX: Convert the timestamp string to an integer for the BIGINT column (u_id)
    ...
    # 2. FIX: Execution is now safe because globals are declared above
    switch(b2,b1,b3,b4)
    preview_image.place_forget()
    names=[]
    ...
    # Clear the previous form/content
    relation.pack_forget() 
    text.pack_forget()
    form.pack_forget()
    ...
    # Define Tkinter StringVars for input fields
    u_name=StringVar()
    pwd=StringVar()
    country=StringVar()
...
def main():
    ...
    text.insert("end", '''Welcome to A.V.I.S. - Analytical Visual Intelligent System!\n\nThis platform is your unified, intelligent solution for data analysis and visualization, designed to automate the entire data-to-insight pipeline.\n\nCore Modules:\n\n1. Automated Data Processing\nPerforms instant data cleaning and Exploratory Data Analysis (EDA) on imported files (CSV, etc.).\n\n2. Dynamic Visualization Engine\nGenerates advanced charts (Bar, Radar, Heatmap, 3D) and recommends the best visual based on data patterns.\n\n3. Predictive & Insight Generation (Research Core)\nProvides Time-Series Forecasting and extracts ranked, automated insights, ready for future Explainable AI (XAI) integration.\n\n\n\n\n\n\n<--- Visit the documentation tab for the Project Architecture''')
